...
function processEachResult(doc) {
  ...
  const sidsFilePath = path.join(__dirname, `../../data/queued-sids-for-${config.broadcastId}-${date}.csv`);
  ...
  return db.collection('messages').aggregate([
    {
      $match: {
        ...
        'metadata.delivery.failedAt': { $exists: 0 },
        'metadata.delivery.deliveredAt': { $exists: 0 },
      },
    },
    {
      $group: {
